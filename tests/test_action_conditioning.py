...
def test_action_conditioned_retrieval_distinguishes_matching_action():
    ...
    def build_memory(scale):
        ...
        matching_id = memory.observe(z=z_common, action=action_a, z_next=None, episode_id=0)
        mismatched_id = memory.observe(z=z_common_near, action=action_b, z_next=None, episode_id=1)
        return memory, matching_id, mismatched_id

    action_memory, matching_id, mismatched_id = build_memory(scale=1.0)
    results = action_memory.recall(z_common, action=action_a, k=2)
    ...
    appearance_memory, matching_id0, mismatched_id0 = build_memory(scale=0.0)
    results0 = appearance_memory.recall(z_common, action=action_a, k=2)
...
def test_determinism_same_seed_same_result():
    ...
    def run():
        ...
        memory = Memory(config, index=FlatIndex(dim=dim + action_dim), action_scale=1.0)
        predictor = ConcatFusionPredictor(eval_adapter, alpha=1.0)
        return evaluate_with_retrieval(memory_episodes, eval_episodes, memory, predictor, k=k)
